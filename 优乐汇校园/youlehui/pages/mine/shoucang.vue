	...
	<view class="shoucang mc26">
		<view class="item_shu_food_sy" v-for="item in hotFoodsList" @click="shopDetail(item)">
			...
			<view class="right">
				...
				<view class="address">
					<text class="iconfont iconlocation"></text>6号楼蓝楼蓝楼蓝楼
				</view>
			</view>
			</view>
			<view class="shoucang_r">
				<view class="shoucang_sy yellow">
...
<script>
	export default {
		...
		methods:{
			...
			shopDetail(item){
				console.log(item)
				uni.setStorageSync('pageTitle','蓝楼')
				uni.switchTab({
					url:"../orders/orders"
				})
			}
