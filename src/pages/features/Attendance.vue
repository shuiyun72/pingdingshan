    ...
    <div class="sign_in_container">
      ...
      <div class="sign_in_button_container">
        <button
          ...
          :disabled="!signInButtonStatus.b1"
          @tap="onSignInClick(1)"
        >
          <i class="fas fa-map-marker-alt fa-2x"></i>
          <div>{{status.state1 === 0 ? '上午签到': '已签到'}}</div>
        </button>
      </div>
      <div class="sign_in_button_container">
        <button
          class="button button-glow button-circle button-action button-jumbo sign_in_button"
          :disabled="!signInButtonStatus.b2"
          @tap="onSignInClick(2)"
        >
          <i class="fas fa-map-marker-alt fa-2x"></i>
          <div>{{status.state2 === 0 ? '上午签退': '已签退'}}</div>
        </button>
      </div>
      <div class="sign_in_button_container">
        <button
          class="button button-glow button-circle button-action button-jumbo sign_in_button"
          :disabled="!signInButtonStatus.b3"
          @tap="onSignInClick(3)"
        >
          <i class="fas fa-map-marker-alt fa-2x"></i>
          <div>{{status.state3 === 0 ? '下午签到': '已签到'}}</div>
        </button>
      </div>
      <div class="sign_in_button_container">
        <button
          class="button button-glow button-circle button-action button-jumbo sign_in_button"
          :disabled="!signInButtonStatus.b4"
          @tap="onSignInClick(4)"
        >
          <i class="fas fa-map-marker-alt fa-2x"></i>
          <div>{{status.state4 === 0 ? '下午签退': '已签退'}}</div>
        </button>
      </div>
    </div>
    <!-- 日期检索 -->
      ...
      >
        ...
        <el-table-column label="状态" align="center">
          <template slot-scope="scope">
            ...
            <span>{{ scope.row.personStatus }}</span>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next(instance => {
      ...
      instance.getAttendanceSchedule();
  ...
  data() {
    return {
      ...
      // 签到表格数据
      tableData: [],
      // 当前（今天）出勤状态 （0：没签  1：已签 ）
      status: { state1: 0, state2: 0, state3: 0, state4: 0 },
      // 当前（今天）出勤状态 （0：没签  1：已签 ）
      signInButtonStatus: { b1: false, b2: false, b3: false, b4: false },
      //上班时间表
      attendanceSchedule: {}
    };
  },
  computed: {
    ...
    reqSubmitAttendance() {
      ...
      let xy = "";
      if (
        this.locationInfo.coords &&
        this.locationInfo.coords.longitude &&
        this.locationInfo.coords.latitude
      ) {
        xy = `${this.locationInfo.coords.longitude},${
          this.locationInfo.coords.latitude
        }`;
      }
      // 请求数据
  ...
  methods: {
    freshSignInButtonsEnabled() {
      this.isSignInButtonEnabled1();
      this.isSignInButtonEnabled2();
      this.isSignInButtonEnabled3();
      this.isSignInButtonEnabled4();
    },
    // 签到1按钮可用状态
    isSignInButtonEnabled1() {
      let dateNow = new Date();
      this.signInButtonStatus.b1 =
        this.attendanceSchedule &&
        dateNow >= this.attendanceSchedule.startCheckInTime1 &&
        dateNow <= this.attendanceSchedule.checkInTime1 &&
        this.status.state1 === 0;
    },
    // 签退1按钮可用状态
    isSignInButtonEnabled2() {
      let dateNow = new Date();
      this.signInButtonStatus.b2 =
        this.attendanceSchedule &&
        dateNow >= this.attendanceSchedule.startCheckOutTime1 &&
        dateNow <= this.attendanceSchedule.checkOutTime1 &&
        this.status.state2 === 0;
    },
    // 签到2按钮可用状态
    isSignInButtonEnabled3() {
      let dateNow = new Date();
      let checkInTime2 = this.attendanceSchedule.checkInTime2;
      this.signInButtonStatus.b3 =
        this.attendanceSchedule &&
        dateNow >= this.attendanceSchedule.startCheckInTime2 &&
        dateNow <= this.attendanceSchedule.checkInTime2 &&
        this.status.state3 === 0;
    },
    // 签退2按钮可用状态
    isSignInButtonEnabled4() {
      let dateNow = new Date();
      this.signInButtonStatus.b4 =
        this.attendanceSchedule &&
        dateNow >= this.attendanceSchedule.startCheckOutTime2 &&
        dateNow <= this.attendanceSchedule.checkOutTime2 &&
        this.status.state4 === 0;
    },
    // 获取考勤记录数据
      ...
      apiInspection
        ...
        .then(res => {
          if (res.data.result === true) {
            ...
            let dateNow = dateHelper.format(new Date(), "yyyy-MM-dd");

            // 计算当前的考勤状态
            mappedRecords.forEach((item, index) => {
              if (item.date === dateNow) {
                this.status[`state${item.state}`] = 1;
              }
            });
          } else {
            // mui.toast(res.data.message);
          }
          this.freshSignInButtonsEnabled();
        });
    },
    getAttendanceSchedule() {
      apiInspection.GetAttendanceSchedule().then(res => {
        let records = res.data;
        let formatedToday = dateHelper.format(this.today, "yyyy-MM-dd ");

        this.attendanceSchedule.checkInTime1 = new Date(
          formatedToday + records.CheckInTime1
        );
        this.attendanceSchedule.checkOutTime1 = new Date(
          formatedToday + records.CheckOutTime1
        );
        this.attendanceSchedule.checkInTime2 = new Date(
          formatedToday + records.CheckInTime2
        );
        this.attendanceSchedule.checkOutTime2 = new Date(
          formatedToday + records.CheckOutTime2
        );

        this.attendanceSchedule.startCheckInTime1 = new Date(
          formatedToday + records.StartCheckInTime1
        );
        this.attendanceSchedule.startCheckOutTime1 = new Date(
          formatedToday + records.StartCheckOutTime1
        );
        this.attendanceSchedule.startCheckInTime2 = new Date(
          formatedToday + records.StartCheckInTime2
        );
        this.attendanceSchedule.startCheckOutTime2 = new Date(
          formatedToday + records.StartCheckOutTime2
        );
    ...
    // 点击打卡按钮
    onSignInClick(attendanceType) {
      if (this.signInButtonStatus[`b${attendanceType}`]) {
        ...
        let attendanceInfo = this.reqSubmitAttendance;
        attendanceInfo.attendanceType = attendanceType;
        attendanceInfo.Lwr_BeiZhu = attendanceType;
        apiInspection.SubmitAttendance(attendanceInfo).then(res => {
          if (res.data.result === true) {
