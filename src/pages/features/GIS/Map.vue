<template>
  <div class="map_container" style="text-align: center">
    ...
    <audio src="static/audio/lingdao.mp3" id="audio" controls></audio>
    <div class="mui-icon audio_btn" :class="IconAduio" @click="audioBtn" v-show="isMissionMap"></div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    mapInstance.on("click", event => {
      ...
      this.presentCoordinate = event.coordinate.join("\r\n");
      console.log("点击当前点位", this.presentCoordinate);
      if (this.currentPickMode === "pick-pipe") {
        ...
        new ol.supermap.QueryService(serviceUrl).queryByDistance(
          ...
          function(serviceResult) {
            ...
            let parsedFeatures = new ol.format.GeoJSON().readFeatures(features);
            // 为选中的feature添加样式
            ...
            let longitude = Number(props["起始点横坐标"] || props["SmSdriE"]);
            let latitude = Number(props["起始点纵坐标"] || props["SmSdriS"]);
            // 设置中心点与缩放
            ...
            const TableColumnConfig =
              ...
              config.superMapIServer.tableColumnConfig["普通给水管线"];
        ...
        new ol.supermap.QueryService(serviceUrl).queryByDistance(
          ...
          function(serviceResult) {
            ...
            let props = feature.properties;
            console.log("props", props);
            let longitude = Number(props["X坐标"]);
            let latitude = Number(props["Y坐标"]);
            let longitudeX = Number(props["SmX"]);
            let latitudeY = Number(props["SmY"]);
            this.mapController.addQueryObjectFeature([
              longitudeX,
              latitudeY
            ]);
            // 设置中心点与缩放
            this.setMapCenterAndZoom([longitudeX, latitudeY], 17);
            // 组装table数据
            const TableColumnConfig =
              config.superMapIServer.tableColumnConfig[props["设备类型"]] ||
              config.superMapIServer.tableColumnConfig[props["类型"]];
            ...
            formattedTableData = [
              {
                propertyName: "Geo",
                propertyValue: longitudeX
              },
              {
                propertyName: "Geo",
                propertyValue: latitudeY
              },
              ...formattedTableData
            ];
            // let formattedTableData = _.map(props, (val, key) => {
            ...
            // });
            this.currentPickedPointDetailData = formattedTableData;
            console.table(this.currentPickedPointDetailData);
            this.$hideLoading();
            this.pointDetailTableVisible = true;
          }.bind(this)
        );
    ...
    if(this.$route.name == "MissionMap"){
       this.isLawful();
       this.isMissionMap = true
    }
  },
  beforeDestroy() {
    ...
    if(this.watchPId){
      window.plus.geolocation.clearWatch(this.watchPId)
      this.watchPId = null;
    }
    if(this.hintTimerInterval){
      clearInterval(this.hintTimerInterval);
      this.hintTimerInterval = null;
    }
  },
  data() {
    return {
  ...
  methods: {
    audioBtn(){ 
      if(this.IconAduio == "mui-icon-mic"){
        this.IconAduio = "mui-icon-micoff";
        this.audioPlay.muted = true;
        this.audioPlay.pause();    
      }else{
        this.IconAduio = "mui-icon-mic";
        this.audioPlay.muted = false;  
      }
    },
    //当前位置是否在巡检区域
    isLawful() {
      ...
      let isArrive = true;
      //区域检测定时器
      this.hintTimerInterval = setInterval(function(){
        time--;
        if(time <= 0){
          time = 10;
          isPosition = true;
      ...
      if (window.plus) {
        let coordsFor84 = [113.2974100112915, 33.74237045645714];
        let _this = this;
        this.watchPId = window.plus.geolocation.watchPosition(({ coords }) => {
          coordsFor84 = CoordsHelper.gcj02towgs84(
            coords.longitude,
            coords.latitude
          );
          _this.onPlanAreaClick(feture => {
            _this.mapController.intersectsCoordinate(
              feture,
              coordsFor84,
              isTure => {
                console.log(isTure);
                //isTure  为ture 代表在巡检区域内 false为区域外
                if(!isTure){
                  if( time == 9 && isPosition ){
                      isArrive = true;
                      isPosition = false;
                      window.mui.toast(`你所在的位置不在巡检区域`);
                      this.audioPlay.src = "static/audio/lingwei.mp3";
                      if(this.IconAduio == "mui-icon-mic"){
                         this.audioPlay.play(); 
                      }        
                  }
                }else{
                  if( time == 9 && isPosition && isArrive){
                    isArrive = false;
                    isPosition = false;                   
                    window.mui.toast(`到达巡检区域`); 
                    this.audioPlay.src = "static/audio/lingdao.mp3";
                    if(this.IconAduio == "mui-icon-mic"){
                      this.audioPlay.play();
                    }
                  }
    ...
    onEventSubmitClick() {
      this.presentCoordinate = this.presentCoordinate || "";
      this.$router.push({
        ...
        query: {
          ...
          presentCoordinate: this.presentCoordinate
        }
      });
    },
    // 初始化地图控制器对象并绘制地图
    ...
    onPlanAreaClick(callback) {
      this.fetchPointsInfo((err, data) => {
        if (err) {
          ...
          callback instanceof Function && callback(f);
        }
      });
