...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next(instance => {
      ...
      //判断路由中是否有位置信息
      if (instance.$route.query.presentCoordinate) {
        locationItem.content = instance.$route.query.presentCoordinate;
        instance.locationInfo.address = locationItem.content.split("\r\n");
        instance.locationInfo.addresses = locationItem.content;
        instance.locationInfo.longitude = locationItem.content.split("\r\n")[0];
        instance.locationInfo.latitude = locationItem.content.split("\r\n")[1];
      } else {
        // 定位
        window.mui.plusReady(() => {
          window.plus.geolocation.getCurrentPosition(
            location => {
              ...
              instance.locationInfo = _.cloneDeep(location);
              // 从items配置项中找到date项
              // let locationItem = _.find(instance.items, item => {
              //   return item.id === "location";
              // });
  ...
  computed: {
    ...
    */
    ...
    currentAddressText() {
      console.log("执行了currentAddressText");
      debugger;
      let addr = this.locationInfo.address;
      console.log(this.locationInfo.addresses);
    ...
    eventSubmissionData() {
      return {
        EventX: this.locationInfo.longitude.toString(),
        EventY: this.locationInfo.latitude.toString(),
        // EventX: "113.199242",
        // EventY: "33.776131",
        iAdminID: this.currentUser.PersonId.toString(),
        cAdminName: this.currentUser.PersonName.toString(),
        iDeptID: this.currentUser.DeptId,
        EventFromId: this.pickerValue.eventSource || "3", 
        ExecDetpID: this.pickerValue.deptList.value,
        ExecPersonId: this.pickerValue.personList.value,
        // 只能上传一张图片
        // 格式化原始的base64字符串为后端能接受的base64格式
        Bae64Image: this.pictureListStr || "",
        //  暂时关闭了麦克风录音功能  by: 2018/06/24
        EventTypeId: this.pickerValue.eventType.value,
        EventTypeId2: this.pickerValue.eventContent.value,
        UrgencyId: this.pickerValue.emergency.value,
        EventDesc: this.detailDescription,
        EventAddress: this.currentAddressText || "平顶山" , 
        LinkMan: this.reporterName || "未填写" ,
        LinkCall: this.reporterMobile || "未填写",
        /*
  ...
  methods: {
    ...
    onRowClick(row, rowIndex) {
      ...
      switch (row.id) {
        ...
        case "personList":
          // 当前已选择的部门的id
          let currentPickedDeptId = this.pickerValue.deptList.value;
          console.log();
          if (!currentPickedDeptId) {
            this.isLoading = false;
            window.mui.toast("请先选择处理部门");
          } else {
            // 请求人员列表
            apiMaintain
              .GetStaffList(currentPickedDeptId)
              .then(res => {
                console.log("// 请求人员列表", res);
                if (res.data.result === true) {
                  let listData = res.data.data;
                  this.openPicker(listData, row, rowIndex, {
                    valueKey: "personId",
    ...
    openPicker(listData, row, rowIndex, keys) {
      ...
      this._popPicker.show(pickedItems => {
        ...
        */
        ...
        if (row.id === "deptList") {
          if (pickedItem.value !== this.pickerValue.deptList.value) {
            _.find(this.items, item => {
              return item.id === "personList";
            }).content = "";
          }
        }
