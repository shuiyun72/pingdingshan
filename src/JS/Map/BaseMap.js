...
function BaseMap() {
  this.center = [109.7526, 39.558];
  this.defaultOptions = _.assign({}, AppConfig.mapInitOption);
  // 地图实例，在Init中初始化
  this.map = null;
  // 用于实时定位的小蓝点
  ...
  var projection_4547 = new ol.proj.Projection({
    code: 'EPSG:4547',
    extent:[344577.88,2381397.91,617340.63,5036050.38],
    units: 'm',
    axisOrientation: 'neu'
  });
  //定义4547坐标系，且与其他4326,3857的互相转换

  proj4.defs("EPSG:4547","+proj=tmerc +lat_0=0 +lon_0=114 +k=1 +x_0=500000 +y_0=0 +ellps=GRS80 +units=m +no_defs");
  
  ol.proj.addProjection(projection_4547);
  ol.proj.addCoordinateTransforms("EPSG:4326", "EPSG:4547",
      function(coordinate) {
  ...
  ol.proj.addCoordinateTransforms("EPSG:3857", "EPSG:4547",
      function(coordinate) {
          return proj4("EPSG:3857","EPSG:4547",coordinate);
      },
      function(coordinate) {
          return proj4("EPSG:4547","EPSG:3857",coordinate);;
      }
  );
...
BaseMap.prototype.SatellMarkLayer; //遥感标记图层
...
BaseMap.prototype.Init = function (containerId, options = {}) {
  ...
  // 遥感标记图层
  this.SatellMarkLayer = new ol.layer.Tile({
    opacity: 1,
    source: new TileSuperMapRest({
      url:  AppConfig.gisData.mark,
      wrapX: true
    }),
    visible: true
  });
  ...
  // 地物查询源
  this.objectQuerySource = new ol.source.Vector();
  // 地物查询矢量图层
  this.objectQueryLayer = new ol.layer.Vector({
      ...
      this.positionFeature.setStyle(
        new ol.style.Style({
          ...
          image: new ol.style.Icon({
            //color: '#4271AE',
            anchor:[0.5,1],
            src: "./static/images/positionPerson.png"
          })
        })
      );
...
BaseMap.prototype.SetMapLayerShow = function (_SetShowLayerIndex, _IsShow) {
  ...
  if (_SetShowLayerIndex == 1) {
    if (_IsShow == 1) {
      ...
      this.SatellMarkLayer.setVisible(true);
      this.StreetLayer.setVisible(false);
      return 2;
    } else {
      this.SatellLayer.setVisible(false);
      this.SatellMarkLayer.setVisible(false);
...
BaseMap.prototype.reachPointWhichId = function (id, color = "#fff", source = this.VectorSource) {
  ...
  var style = new ol.style.Style({
    image: new ol.style.Icon(option)
  });
  f.setStyle(style)
}
 ...
 * coordinate 点位的坐标
 */
BaseMap.prototype.intersectsCoordinate = function(feture,coordinate,callback){
  console.log("intersectsCoordinate",feture)
  console.log("intersectsCoordinate",coordinate)
  var isIn = feture.getGeometry().intersectsCoordinate(coordinate)
  callback instanceof Function && callback(isIn);
