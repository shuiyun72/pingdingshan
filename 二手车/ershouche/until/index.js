...
let apiUrl = 'http://escar.shxin.top';
Vue.prototype.$apiUrl = 'http://escar.shxin.top';
...
Vue.prototype.$getApi = function(url, data, callsuc, token,err) {
	...
	if (token && userInfo) {
		data.token = JSON.parse(userInfo).token;
		data.type = JSON.parse(userInfo).type;
		data.page = 1;
		data.paginate = 200;
	}
	...
	uni.request({
		...
		success: (res) => {
			...
			if (res.statusCode == 401 || res.statusCode == 500 || res.statusCode == 501) {
				this.$msg("请重新登录")
				// #ifndef MP
